...
Repo.prototype.install = function() {
    ...
    return fs.remove(this._id + '.app').catch(err => null)
        .then(() => exec('./', 'git -c http.sslVerify=false clone $ $ --single-branch -b $ --depth 1 --quiet', this.repo, this._id + '.app', this.branch))
...
Repo.prototype.update = function() {
    ...
    return Promise.resolve()
        .then(() => exec(this._id + '.app', 'git -c http.sslVerify=false fetch --quiet origin $', this.branch))
        .then(() => exec(this._id + '.app', 'git reset --hard origin/$', this.branch))
        .then(() => this.installDep())
        .then(() => this.startup())
};
...
Repo.prototype.installDep = function() {
    ...
    return exec('.', 'which cnpm')
        .then(cnpm => exec(this._id + '.app', (cnpm.out ? 'cnpm' : 'npm') + ' install --loglevel win'));
};
    ...
    return fs.mkdir(this._id + '.log').catch(err => null)
        ...
        .then(() => {
            ...
            const child = this.child = cp.spawn(
                process.argv[0], 
                [path.resolve(this._id + '.app', this.run), '--port', portNum].concat(
                    splitargs(this.args.replace(/\$\{app_dir\}/g, path.resolve(this._id + '.app')))
                ),
                { cwd: path.resolve(this._id + '.cwd'), stdio: ['ignore'] }
            );
