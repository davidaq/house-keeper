...
const svnArgs = ' --force -q --trust-server-cert --non-interactive ';
...
Repo.prototype.setOpt = function(opt) {
    this._id = opt._id;
    this.repo = (opt.repo || 'x').trim();
    this.branch = opt.branch;
    this.run = opt.run;
    this.args = opt.args;
    this.wdir = (opt.wdir || './').trim();
    this.type = opt.type;
...
Repo.prototype.installOrUpdate = function() {
    return fs.readFile(path.join(this.dir('app'), '.' + this.type, '.housekeeper-repo-url'), 'utf8')
        .catch(() => '')
        .then(url => {
            if (url.trim() == this.repo) {
                return this.update();
            } else {
                return this.install();
            }
    ...
    return fs.mkdirs(this.dir('log')).catch(err => null)
        ...
        .then(() => {
            ...
            case 'git':
                ...
                return exec(this.dir(), 'git -c http.sslVerify=false clone $ $ -b $ --quiet', this.repo, 'app', this.branch);
            case 'svn':
        ...
        .then(() => fs.writeFile(path.join(this.dir('app'), '.' + this.type, '.housekeeper-repo-url'), this.repo, 'utf8'))
...
Repo.prototype.update = function() {
    ...
    case 'svn':
        ...
        return Promise.resolve()
            ...
            .then(() => {
                if (this.username && this.password) {
                    return exec(this.dir('app'), 'svn up' + svnArgs + '--username $ --password $', this.username, this.password);
                } else if (this.username) {
                    return exec(this.dir('app'), 'svn up' + svnArgs + '--username $', this.username);
                } else {
                    return exec(this.dir('app'), 'svn up' + svnArgs);
                }
            })
            .then(() => this.installDep())
            .catch(err => null)
...
Repo.prototype.installDep = function() {
    ...
    var appDir = path.join(this.dir('app'), this.wdir);
    return fs.exists(path.join(appDir, 'package.json'))
        .then(exists => {
            return exists && exec(appDir, 'npm install --only=prod --loglevel win');
        });
};
    ...
    return fs.mkdirs(this.dir('log')).catch(err => null)
        ...
        .then(() => {
            ...
            if (!this.run) {
                ...
                const args = [path.join(this.dir('app'), this.wdir, this.run), '--port', portNum].concat(
                    splitargs(this.args
